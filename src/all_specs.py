...
def obtener_dispositivos_red():
    ...
    dispositivos = []
    datos = []
    # Buscar en el directorio output/
    output_dir = Path(__file__).parent.parent / "output"
    datos = csv_a_lista_dicts(buscar_por_patron(output_dir))
    for fila in datos:
        ip, mac = fila.items()
        dispositivos.append(
            {
...
class AllSpecs(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # ­ƒÜÇ iniciar actualizaci├│n peri├│dica
        ...
        if isinstance(event, ActualizarTablaEvent):
            if event.error:
                ...
                self.statusbar.showMessage(
                    f"├Ültima actualizaci├│n: {datetime.now().strftime('%H:%M:%S')}"
                )
    ...
    def mostrar_dispositivos(self, dispositivos):
        ...
        for fila, d in enumerate(dispositivos):
            ...
            estado_item = QTableWidgetItem("­ƒƒó" if d.get("activo") else "­ƒö┤")
